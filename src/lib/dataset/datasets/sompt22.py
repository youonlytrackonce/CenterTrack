...
class sompt22(GenericDataset):
  ...
  def __init__(self, opt, split):
    ...
    print('==> initializing SOMPT22 {} data.'.format(split))
  ...
  def save_results(self, results, save_dir):
    self._save_results(self.convert_eval_format(results),
                       '{}/results_sompt22.json'.format(save_dir))
  def run_eval(self, results, save_dir):
    self.save_results(results, save_dir)
    try:
      os.system('python tools/sompt22_eval/demo.py ' + \
                '../data/sompt22/annotation/val.json ' + \
                '{}/results_sompt22.json'.format(save_dir))
    except:
      print('sompt22 evaluation not setup!')
